...
from (select recent_days,
     ...
     (select recent_days,
      ...
      where ds = '2025-07-17') dispatch_nd
     ...
     (select '2025-07-17'                                         ds,
      ...
      from (select if(org_level = 1, city_for_level1.id, city_for_level2.id)     city_id,
            ...
            from (select org_id,
                         ...
                         trans_finish_dur_sec
                  from dws_trans_org_truck_model_type_trans_finish_1d
                  where ds = '2025-07-17') trans_origin
                     left join
                 (select id,
                         org_level,
                         region_id
                 ...
                 (select id,
                         name
                  from dim_region_full
                  where ds = '20250717') city_for_level2
...
from (select '2025-07-17'     ds,
      ...
      where ds = '2025-07-17') deliver_1d
         full outer join
     (select '2023-01-10'    ds,
             1               recent_days,
             sum(sort_count) sort_count
      from dws_trans_org_sort_1d
      where ds = '2025-07-17') sort_1d
