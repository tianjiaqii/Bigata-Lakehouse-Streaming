...
from (select order_id,
      ...
      from (select order_id,
           ...
           (select id org_id,
                   ...
                   region_id
            from dim_organ_full
            where ds = '20250717') organ
           on detail.sender_district_id = organ.region_id
               left join
           (select id,
                   parent_id
            from dim_region_full
            where ds = '20250717') district
...
from (select id,
      ...
      where ts = '20250717') trans_finish
         left join
     (select id,
             truck_model_type,
             truck_model_type_name
...
from (select org_id,
     ...
     (select id,
             ...
             parent_id
      from dim_region_full
      where ds = '20250717') city_for_level1
     ...
     (select id,
             ...
             parent_id
      from dim_region_full
      where ds = '20250717') province_for_level1
     ...
     (select id,
             ...
             parent_id
      from dim_region_full
      where ds = '20250717') province_for_level2
...
where ds >= date_add('2025-07-17', -recent_days + 1)
group by org_id,
         org_name,
         city_id,
         city_name,
...
from (select recent_days,
      ...
      from dwd_trans_trans_finish_inc lateral view
          explode(array(7, 30)) tmp as recent_days
      where ts >= date_add('20250717', -recent_days + 1)
      group by recent_days,
               shift_id,
               line_id,
               start_org_id,
     ...
     (select id,
             line_name
      from dim_shift_full
      where ds = '20250717') for_line_name
...
from dws_trans_org_deliver_suc_1d lateral view
    explode(array(7, 30)) tmp as recent_days
where ds >= date_add('20250717', -recent_days + 1)
group by org_id,
         org_name,
         city_id,
         city_name,
...
from (select order_count,
      ...
      where ds = date_add('20250717', -1)
      union
      select order_count,
             order_amount
      from dws_trans_dispatch_1d
      where ds = '20250717') all_data;
...
create external table dws_trans_bound_finish_td(
    ...
    partitioned by (`ds` string comment '统计日期')
    stored as orc
    location 'hdfs://cdh01:8020/warehouse/tms/dws/dws_trans_bound_finish_td'
    tblproperties('orc.compress'='snappy');
